...
namespace
{
    ...
    struct StudioWrapper : public Wrapper
    {
		
		StudioWrapper()
			: Wrapper(pipeline::wif()) {

			//get the producer out of wrapped
			auto producer = std::dynamic_pointer_cast<Processor<std::shared_ptr<imaging::Bitmap>, std::shared_ptr<std::vector<uint8_t>>>>(wrapped);
				
			//initalize base64
			auto base64 = pipeline::base64();
			
			//link base64 to wif
            producer->link_to(base64);
			
			//initalize stdout
            auto stdout = pipeline::text_writer(std::cout);

			//link base64 to stdout
			base64->link_to(stdout);
        ...
        void consume(Boxed_Value boxed) override
        {
			auto unboxed = boxed_cast<std::shared_ptr<Bitmap>>(boxed);
			auto consumer = std::dynamic_pointer_cast<Consumer<std::shared_ptr<Bitmap>>>(this->wrapped);
    ...
    struct PipelineLibrary
    {
        ...
        std::shared_ptr<Wrapper> studio() const
        {
		    ...
		    //return instantie van studiowrapper
			return std::make_shared<StudioWrapper>();
        }
